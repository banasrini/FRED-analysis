...
def add_cut_overlays(fig, cut_dates, window_months=12,
    ...
    for i, dt in enumerate(cut_dates):
        ...
        # Split add_vline and add_annotation — passing annotation_* kwargs
        # inline to add_vline causes a TypeError in older Plotly versions when
        # the x-axis contains date strings (Plotly's internal _mean() does
        # sum([0, "2020-01-01"]) which fails with 'int' + 'str').
        fig.add_vline(
            x=x0_str,
